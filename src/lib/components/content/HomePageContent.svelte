<script lang="ts">
  ...
  const heroFallbackImage = '/images/brand/alderway-hero.webp';
...
{#each visibleSections as section (section.id)}
  {#if section.type === 'hero'}
    <section class="relative flex min-h-[72vh] max-h-[820px] items-center overflow-hidden md:min-h-[82vh]">
      <div class="absolute inset-0">
        ...
        <div class="absolute inset-0" style="background:linear-gradient(to right, rgba(15,35,29,0.82) 0%, rgba(15,35,29,0.6) 34%, rgba(15,35,29,0.12) 68%, rgba(15,35,29,0.02) 100%)"></div>
        <div class="absolute inset-0 bg-[rgba(31,58,50,0.06)]"></div>
      </div>
      <div class="container-x relative w-full">
        <div class="max-w-2xl text-left">
          <span class="mb-5 block text-[11px] font-semibold uppercase tracking-[0.24em] text-[#e1ad8a]">Alderway Campers</span>
          <h1 class="mb-6 max-w-2xl font-serif text-[42px] font-semibold leading-[0.98] tracking-[-0.035em] text-white sm:text-[58px] md:text-[76px]" style="text-shadow:0 2px 22px rgba(0,0,0,0.38)">
            {#each tx(section.title).split('\n') as line}<span class="block">{line}</span>{/each}
          </h1>
          {#if tx(section.body)}<p class="mb-9 max-w-xl text-[15px] leading-7 text-white/90 md:text-[17px]" style="text-shadow:0 1px 10px rgba(0,0,0,0.38)">{tx(section.body)}</p>{/if}
          {#if section.ctaHref && tx(section.ctaLabel)}<a href={section.ctaHref} class="btn btn-primary px-8 py-4">{tx(section.ctaLabel)} <span aria-hidden="true">→</span></a>{/if}
        </div>
      </div>
    </section>
  {:else if section.type === 'vehicle_grid'}
    ...
    <section class="section home-rentals-section">
      <div class="container-x">
        ...
        {#if section.ctaHref && tx(section.ctaLabel)}<p class="mt-10 text-center text-[14px] text-[#7a7f86] reveal">{tx(section.body)} <a href={section.ctaHref} class="font-semibold text-[#9f542e] hover:underline">{tx(section.ctaLabel)}</a></p>{/if}
      </div>
    </section>
    ...
    <section class="bg-[#f6f7f9] py-10">
      <div class="container-x"><div class="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
        {#each section.items ?? [] as item}
          <a href={item.href || '#'} target="_blank" rel="noreferrer" class="flex h-28 flex-col items-center justify-center gap-2 rounded-lg border border-[#ededf0] bg-white px-4 text-center transition hover:-translate-y-0.5 hover:border-[#c87442]" aria-label={tx(item.title)}>
        ...
        <div class="grid grid-cols-1 gap-10 md:grid-cols-3 md:gap-8">
          {#each section.items ?? [] as item}
            <div class="px-4 text-center reveal">
              <div class="mb-5 flex items-center justify-center text-[#c87442]">
    ...
    <section class="overflow-hidden py-4">
      <div class="marquee">
        <div class="image-marquee-track">
          {#each [0, 1] as groupIndex}
            <div class="image-marquee-group" aria-hidden={groupIndex === 1 ? 'true' : undefined}>
              {#each [0, 1, 2] as repeatIndex}
                {#each section.items ?? [] as item}
                  <div class="h-[200px] w-[280px] flex-shrink-0 overflow-hidden bg-[#f3f4f6]">
                    {#if item.image}<img src={item.image} alt={groupIndex === 0 && repeatIndex === 0 ? tx(item.alt) : ''} loading="lazy" class="h-full w-full object-cover transition-transform duration-700 hover:scale-105" />{/if}
                  </div>
                {/each}
              {/each}
            </div>
  ...
  {:else if section.type === 'stats'}
    <section id="stats" class="bg-[#1f3a32] py-16"><div class="container-x"><div class="grid grid-cols-1 gap-10 text-center md:grid-cols-3">
      {#each section.items ?? [] as item}<div><div class="mb-3 font-serif text-6xl font-semibold leading-none text-white md:text-7xl">{counts[item.id] ?? 0}{item.suffix ?? ''}</div><div class="text-[12px] font-bold uppercase tracking-[0.16em] text-[#e1ad8a]">{tx(item.title)}</div></div>{/each}
    </div></div></section>
  {:else if section.type === 'logo_marquee'}
    <section class="overflow-hidden bg-[#fafbfc] py-12"><div class="marquee"><div class="marquee-track gap-4">
      {#each [...(section.items ?? []), ...(section.items ?? [])] as item}
        <a href={item.href || '#'} target="_blank" rel="noreferrer" class="flex h-[110px] w-[200px] flex-shrink-0 flex-col items-center justify-center gap-2 rounded-lg border border-[#ededf0] bg-white px-6 text-center transition hover:border-[#c87442]" aria-label={tx(item.title)}>
...
<style>
  ...
  .image-marquee-track {
    display: flex;
    width: max-content;
    animation: image-marquee-scroll 42s linear infinite;
  ...
  .marquee:hover .image-marquee-track {
