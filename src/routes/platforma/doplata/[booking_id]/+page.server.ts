...
export const load: PageServerLoad = async ({ params }) => {
  const [{ data: booking }, { data: rows }] = await Promise.all([
    supabaseAdmin.from('bookings')
      .select('id,confirmation_number,driver_name,second_payment_amount,second_payment_due_date,second_payment_status,payment_split')
      .eq('id', params.booking_id).single(),
    supabaseAdmin.from('settings').select('key,value').in('key', ['ibans', 'company'])
  ]);
  if (!booking?.payment_split) throw error(404, 'Doplata nije pronađena.');
  ...
  return { booking, bankTransfers, corvuspayAvailable: corvuspayAvailable() };
};
