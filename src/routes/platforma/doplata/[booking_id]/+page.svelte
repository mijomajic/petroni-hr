...
<svelte:head>
  <title>Doplata rezervacije — {BUSINESS.shortName}</title>
...
<div class="min-h-[70vh] flex items-center justify-center py-20 px-4">
  <div class="text-center max-w-6xl w-full">
    <div class="w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-8" style="background:#f5e8df;border:2px solid #fbe7a1">
      <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="#c87442" stroke-width="2.5" aria-hidden="true"><path d="M12 2v20M17 6.5H9.5a3.5 3.5 0 0 0 0 7H14a3.5 3.5 0 0 1 0 7H6" /></svg>
    </div>
    ...
    {:else}
      <div class="text-left card card-static p-5 mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-5">
        ...
        {:else}
          <p class="text-sm text-[#8b9099]">Kartično plaćanje trenutačno nije dostupno za ovu rezervaciju.</p>
        {/if}
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        {#each data.bankTransfers as transfer}
          <div class="card card-static p-5 text-left min-w-0">
            ...
            <img class="w-full h-32 object-contain mt-4 bg-white" src={transfer.barcode} alt="Bank transfer payment code for {transfer.bank}" />
          </div>
        {/each}
      </div>
    {/if}
