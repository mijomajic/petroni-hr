...
export const POST: RequestHandler = async ({ request, url }) => {
  ...
  const { data: booking } = await supabaseAdmin.from('bookings')
    .select('id,confirmation_number,driver_email,first_payment_amount,second_payment_amount')
    .eq('id', bookingId).single();
  if (!booking) throw error(404);
  ...
  const response = `<!doctype html><html><body><form id="pay" method="post" action="${redirectData.url}">${Object.entries(redirectData.fields).map(([key, value]) => `<input type="hidden" name="${key}" value="${value.replaceAll('&', '&amp;').replaceAll('"', '&quot;')}">`).join('')}</form><script>document.getElementById('pay').submit()</script></body></html>`;
