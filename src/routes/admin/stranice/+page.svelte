...
<svelte:head><title>Javne stranice — Admin — {BUSINESS.shortName}</title></svelte:head>
      ...
      <article class="grid gap-4 px-5 py-5 md:grid-cols-[1fr_auto] md:items-center">
        <div>
          <div class="flex flex-wrap items-center gap-3">
            ...
            {#if page.uses_fallback}<span class="rounded bg-[#f5e8df] px-2 py-1 text-[10px] font-bold uppercase tracking-wide text-[#704025]">Početni sadržaj</span>{/if}
          </div>
        ...
        <div class="flex flex-wrap gap-2">
          ...
          <a href="/admin/stranice/{page.key}" class="rounded-md bg-[#c87442] px-4 py-2.5 text-xs font-black text-[#2b2b2b] hover:bg-[#e8b900] active:-translate-y-px">Uredi stranicu</a>
        </div>
      </article>
    {/each}
  </div>

  <div class="mt-8 border-l-2 border-[#c87442] bg-[#fffdf5] px-5 py-4 text-sm leading-6 text-[#6f5600]">
