...
<div>
  <div class="flex items-center justify-between mb-8">
    <h1 class="text-3xl font-black uppercase tracking-tight text-white">Proizvodi</h1>
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-sm">
          <thead style="border-bottom: 1px solid #2a2a2a">
            <tr>{#each ['Proizvod', 'SKU', 'Cijena', 'Zaliha', 'Status'] as h}<th class="text-left px-4 py-3 text-xs uppercase tracking-widest font-bold" style="color: #9ca3af">{h}</th>{/each}</tr>
          </thead>
          <tbody>
            {#each products as p}
              <tr style="border-bottom: 1px solid #1a1a1a">
                <td class="px-4 py-3">
                  <div class="flex items-center gap-3">
                    ...
                    <div><p class="font-medium text-white">{p.name_hr}</p></div>
                  </div>
                </td>
                <td class="px-4 py-3 font-mono text-xs" style="color: #9ca3af">{p.sku || '—'}</td>
        ...
        {#if products.length === 0}<p class="p-8 text-center text-sm" style="color: #9ca3af">Nema proizvoda.</p>{/if}
      </div>
    {/if}
  </div>
</div>
