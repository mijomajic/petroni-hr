...
<div>
  <div class="flex items-center justify-between mb-8">
    <h1 class="text-3xl font-black uppercase tracking-tight text-white">Rezervacije</h1>
    <select class="px-4 py-2 rounded-xl text-sm text-white focus:outline-none" style="background: #1a1a1a; border: 1px solid #2a2a2a" bind:value={filterStatus}>
  ...
  <div class="rounded-2xl overflow-hidden" style="background: #111; border: 1px solid #1a1a1a">
    {#if loading}
      <div class="p-8 space-y-3">
        {#each [1,2,3,4,5] as _}<div class="h-12 rounded-xl animate-pulse" style="background: #1a1a1a"></div>{/each}
      </div>
    {:else}
      <div class="overflow-x-auto">
          ...
          <tbody>
            {#each filtered as b}
              <tr style="border-bottom: 1px solid #1a1a1a">
                <td class="px-4 py-3">
                  <p class="text-white font-medium">{b.driver_name}</p>
                  <p class="text-xs" style="color: #9ca3af">{b.driver_email}</p>
                </td>
                <td class="px-4 py-3 text-white">{b.vehicles?.name ?? '—'}</td>
                <td class="px-4 py-3" style="color: #9ca3af">{b.pickup_date}</td>
                <td class="px-4 py-3" style="color: #9ca3af">{b.dropoff_date}</td>
                ...
                <td class="px-4 py-3">
                  <span class="px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-widest"
                    style="{b.status === 'confirmed' ? 'background: rgba(22,163,74,0.15); color: #16a34a' : b.status === 'pending' ? 'background: rgba(245,197,24,0.15); color: #F5C518' : b.status === 'cancelled' ? 'background: rgba(239,68,68,0.1); color: #ef4444' : 'background: rgba(100,100,100,0.15); color: #9ca3af'}">
                    {b.status}
                  </span>
                </td>
                <td class="px-4 py-3">
                  <select
                    class="px-3 py-1.5 rounded-xl text-xs text-white focus:outline-none"
        ...
        {#if filtered.length === 0}
          <p class="p-8 text-sm text-center" style="color: #9ca3af">Nema rezervacija.</p>
        {/if}
      </div>
    {/if}
  </div>
